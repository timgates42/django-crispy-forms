...
* The `flat_attrs()` method of `FormActions` is removed. Attributes provided by `**kwargs` are now passed via the 
  `crispy_forms.utils.flat_attrs()` function during `__init__()` instead of with each call of `render()`.
  The function in the `utils` module can be used if required.
